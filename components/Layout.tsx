...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, userPoints = 0 }) => {
  ...
  return (
    <div className="flex flex-col h-[100dvh] max-w-md mx-auto relative overflow-hidden bg-gray-50 dark:bg-gray-950">
      ...
      <main className="flex-1 overflow-y-auto px-4 pt-4 pb-20 no-scrollbar">
        {children}
      </main>

      <nav className="shrink-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 safe-area-bottom">
        <div className="flex justify-around items-center h-16">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => handleTabChange(item.id as Tab)}
              className="flex flex-col items-center justify-center w-full h-full space-y-1 transition-all active:opacity-60"
            >
              {item.icon(activeTab === item.id)}
              <span className={`text-[9px] font-bold uppercase ${activeTab === item.id ? 'text-[#1b5e20]' : 'text-gray-400'}`}>
                {item.label}
              </span>
            </button>
          ))}
